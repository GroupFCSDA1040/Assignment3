...
After loading both our population and energy datasets, we need to merge the population data into our energy data based off of the date column.
```{r, echo=FALSE, include=FALSE}
View(pop_us)
View(AEP_energy)
...
```{r}
ggplot(AEP_energy, aes(Date, mw)) + geom_line() + scale_x_date(labels = date_format("%Y"), breaks='1 years') + ylab("Cleaned Enery Consumption (MW)") + xlab("Year")
```
...
```{r}
#na_index=which(is.na(AEP_energy$AEP_MW))
#mean_ai=mean(AEP_energy$AEP_MW,na.rm=TRUE)
#print(mean_ai)
#AEP_energy$AEP_MW[na_index]=mean_ai
#summary(AEP_energy$AEP_MW)
#View(AEP_energy)
```
...
----------------------------------------This is where I started the VAR analysis. 
...
saveRDS(AEP_energy_var, file = "VARmodel.Rds")
AEP_energy_var$p
```
...
```
```{r}
autoplot( predict(AEP_energy_var, n.ahead = 10))
```
...
# plot(fit1, which = 3)
# plot(dccf1, which = 3)
saveRDS(fit1, file = "DCCGarch_model.Rds")
```
